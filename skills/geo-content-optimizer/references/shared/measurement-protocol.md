...
When a workflow needs a machine-verifiable cross-discipline loop, encode the preregistration as `kind: measurement-contract` under [`control-artifact.schema.json`](https://github.com/aaron-he-zhu/aaron-marketing-skills/blob/5a1b2360eb9b61e8fb94e9403ff5f399ff40f765/references/control-artifact.schema.json). Bind the exact measured target, candidate, and control bytes; population/scope; analysis unit; primary metric and guardrails; truth source; attribution and conversion-lag references; locked/start/stop/read times; decision rule; and decision owner. `locked_at` must be no later than exposure, and confirmatory designs require a control. `none-exploratory` is explicitly exploratory and cannot be reported as confirmatory evidence.
...
Use [`scripts/connectors/experiment.py`](https://github.com/aaron-he-zhu/aaron-marketing-skills/blob/5a1b2360eb9b61e8fb94e9403ff5f399ff40f765/scripts/connectors/experiment.py) for stdlib deterministic calculations. Inputs are user-provided observations; p-values, intervals, power, and effect estimates are **Calculated**. The helper deliberately emits no `promote`, `rollback`, or winner verdict.
...
- **Influencer** — content has a short head (first 24–48h decides most reach) then a long tail; judge a post on its own platform analytics vs the creator's recent median, not vs another creator. Return rolls up into the SQS per the [STAR framework](https://github.com/aaron-he-zhu/aaron-marketing-skills/blob/5a1b2360eb9b61e8fb94e9403ff5f399ff40f765/references/star-benchmark.md).
...
CORE-EEAT, CITE, STAR, ROAS, SEND, RAMP, ECHO, and TALE scores are advisory quality-control summaries. Deterministic math guards internal consistency; it does not establish reliability or external validity. Outcome calibration must follow the preregistered, version-locked study in [`scoring-semantics.md`](https://github.com/aaron-he-zhu/aaron-marketing-skills/blob/5a1b2360eb9b61e8fb94e9403ff5f399ff40f765/references/scoring-semantics.md). Until that study succeeds for a named profile/population/outcome, present scores as structured audit evidence, not outcome predictors.

See also: [CONNECTORS.md](https://github.com/aaron-he-zhu/aaron-marketing-skills/blob/5a1b2360eb9b61e8fb94e9403ff5f399ff40f765/CONNECTORS.md) (the data recipes and the measurement-loop note),
[scripts/connectors/README.md](README.md) (helper reference),
[state-model.md](https://github.com/aaron-he-zhu/aaron-marketing-skills/blob/5a1b2360eb9b61e8fb94e9403ff5f399ff40f765/references/state-model.md) (where snapshots live in `memory/`).
