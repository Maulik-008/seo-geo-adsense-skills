...
> Emit the standard shape from [skill-contract.md §Handoff Summary Format](references/shared/skill-contract.md).

## Data Sources

Use `~~AI monitor` and `~~SEO tool` when connected; otherwise ask for target queries, content, engines, competitor examples, and known AI-citation gaps. See [CONNECTORS.md](https://github.com/aaron-he-zhu/aaron-marketing-skills/blob/5a1b2360eb9b61e8fb94e9403ff5f399ff40f765/CONNECTORS.md).

**Measuring whether GEO work paid off:** the changes this skill makes (extractable, quotable, answer-shaped content) move **citability** — testable in minutes by handing a live-fetch engine the URL and asking the target query. That is a *proxy*. Whether an engine then cites you *unprompted* (surfacing) is gated by its crawl/index refresh — week-scale and confounded, not minute-level. Do not conflate the two or promise fast surfacing. The latency of each signal, and why outcome deltas need a control group, are defined in [references/measurement-protocol.md](references/shared/measurement-protocol.md).

**Keyless AI-citation probe (Tavily)**: `python "<skill-dir>/scripts/tavily.py" search "<target query>" --answer --limit 10` (bundled; this sends the query to api.tavily.com, so tell the user before running it) runs that citability test against a real AI answer engine, no key needed — check whether the synthesized answer cites your URL/domain and where the page ranks among the scored sources. This is **Measured** for Tavily's own layer and an **Estimated proxy** for ChatGPT/Perplexity/Google AI Overviews (different indexes, different retrieval). Re-run after shipping changes for a minutes-scale citability read; unprompted surfacing stays week-scale per the paragraph above. See [scripts/connectors/README.md](references/shared/README.md).

## Local install rules (override the steps below)

A self-contained copy of upstream v20.1.0; see [LOCAL-CHANGES.md](LOCAL-CHANGES.md). In this skill set the goal is quality content that passes AdSense review, so:

1. **Sourced or nothing.** "Sourced quotable statements", "expert signals" and "specific data" mean facts from a real, linkable source, or the author's own real material. Never invent a statistic, quote, expert, study, or date to raise a GEO score. If a gap needs data you do not have, leave `[TK: …]` and ask.
2. **human-web-content wins on style.** After optimizing, run its §9 anti-slop pass as well as the humanizer check. GEO structure (definitions, tables, Q&A) is welcome. Performed "authoritative tone", keyword-heavy definitions, and uniform 2–3-sentence paragraphs are not.
3. **FAQ schema has no rich-result promise.** Add FAQ markup only for real questions that are visible on the page (see `serp-markup-builder`).
4. **The memory/ writes in this contract happen only when the user confirms.**
5. When this skill runs inside an AdSense push, hand back to [adsense-content-pipeline](../adsense-content-pipeline/SKILL.md) after the auditor.
...
5. **CORE-EEAT GEO Self-Check** — verify C02, C04, C09, O02, O03, O05, O06, R01, R02, R04, R07, E01, Exp10, Ept08 with Pass/Warn/Fail. Then run the [slop self-check](references/shared/humanizer-slop.md) to strip AI-tell phrasing before the asset ships.
...
On user confirmation, save to `memory/content/YYYY-MM-DD-<topic>.md` — see [Skill Contract](references/shared/skill-contract.md) §Save Results Template.
...
- [Slop Self-Check](references/shared/humanizer-slop.md) - Pre-publish pass to strip AI-tell phrasing before content ships
- [Agent-Readable File Stack (llms.txt / OKF)](references/shared/llms-txt-okf.md) - Machine-readable files so agents and engines can parse your site
- [Grokipedia Tactics](references/shared/grokipedia.md) - AI-citation tactics for Grok / Grokipedia
- **GEO distribution surfaces** — platform refs engines pull from: [X](references/shared/x.md), [LinkedIn](references/shared/linkedin.md), [YouTube](references/shared/youtube.md), [Reddit](references/shared/reddit.md)
...
**Termination note**: keep a visited-set this session; if the recommended skill was already invoked, stop and report the chain complete rather than re-running it. Honor a max handoff depth of 3 to avoid loops (per [skill-contract.md §Termination rules](references/shared/skill-contract.md)).
