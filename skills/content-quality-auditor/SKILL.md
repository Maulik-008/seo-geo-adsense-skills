...
### Local install (read first)

This is a self-contained copy of upstream v20.1.0 (commit `5a1b236`); see [LOCAL-CHANGES.md](LOCAL-CHANGES.md). The root runtime files are bundled inside this skill, and the scorer plus catalog are pinned here, so audits **can** be scored:

- `AARON_SKILLS_ROOT` = **this skill's base directory**. Wherever the shared docs say `$AARON_SKILLS_ROOT/scripts/rubric-score.py` or `$AARON_SKILLS_ROOT/references/framework-catalog.json`, those files exist at that path in this folder.
- Do not resolve `CLAUDE_PLUGIN_ROOT` or the git top level, and skip the plugin profile-resolver steps in `runtime-invocation.md`; they apply only to the full plugin.
- The artifact validator is not bundled, so the **Persistence** section does not apply: report audits inline and never write under `memory/audits/`.
- On Windows use `python` if `python3` is not available.
- Read [local-overrides.md](references/local-overrides.md) before scoring. It keeps scoring honest when these audits feed the `adsense-content-pipeline`.

### Runtime Reads

- [`auditor-runbook.md`](references/shared/auditor-runbook.md)
- [`scoring-semantics.md`](references/shared/scoring-semantics.md)
- [`core-eeat-benchmark.md`](references/shared/core-eeat-benchmark.md)
- [`framework-catalog.json`](references/framework-catalog.json)
- [`audit-run.schema.json`](references/shared/audit-run.schema.json)
- [`local-overrides.md`](references/local-overrides.md)
- `references/auditor-runtime.md` (compact fallback only)

### Runtime Contract

At activation, read the bundled files:

1. `references/shared/auditor-runbook.md`
2. `references/shared/scoring-semantics.md`
3. `references/shared/core-eeat-benchmark.md`
4. `references/framework-catalog.json` (`CORE-EEAT` entry)
5. `references/local-overrides.md`

Never fetch a mutable branch or continue with a guessed contract. Before scoring, verify that `<skill-dir>/scripts/rubric-score.py` and `<skill-dir>/references/framework-catalog.json` exist, and run `python "<skill-dir>/scripts/rubric-score.py" check-catalog` once per session. If either file is missing or the catalog check fails, fall back to `references/auditor-runtime.md` and return `score_state: NOT_SCORED` / `score_confidence: not_scored` with no gate verdict. Record `schema_version: 3.0`, `runbook_version: 3.0.0`, and catalog version in the report.
...
6. Create a JSON run conforming to `audit-run.schema.json` and execute `python "<skill-dir>/scripts/rubric-score.py" score <run.json>` (the bundled scorer; write `run.json` to a temp/scratch location, not the user's repo). Preserve the typed input and output for reproducibility.
...
**In this install:** `content-writer` → `human-web-content`; `technical-seo-checker` / `on-page-seo-checker` → `seo-geo` (plus the crawl section of `adsense-auditor`); head markup → `serp-markup-builder`. `offer-claims-registry` and `domain-authority-auditor` are not installed, so report those items as open loops. When the audit is part of an AdSense push, hand back to `adsense-content-pipeline`.
...
> **Not available in this install.** The validator script is not bundled, so never persist audit artifacts; report inline. The upstream text below is kept for reference.
...
- [CORE-EEAT benchmark](references/shared/core-eeat-benchmark.md)
- [Auditor runbook](references/shared/auditor-runbook.md)
- [Scoring semantics](references/shared/scoring-semantics.md)
- [Item reference](references/item-reference.md)
- [Recursive refinement](references/recursive-refinement.md)
- [Humanizer controls](references/shared/humanizer-slop.md)

## Next Best Skill

- **FIX content:** [human-web-content](../human-web-content/SKILL.md) (local stand-in for upstream `content-writer`)
- **AdSense push:** [adsense-content-pipeline](../adsense-content-pipeline/SKILL.md)
- **FIX technical evidence:** [technical-seo-checker](../seo-geo/SKILL.md)
- **Resolve claims:** [offer-claims-registry](https://github.com/aaron-he-zhu/aaron-marketing-skills/blob/5a1b2360eb9b61e8fb94e9403ff5f399ff40f765/protocol/offer-claims-registry/SKILL.md)
- **Add domain context:** [domain-authority-auditor](https://github.com/aaron-he-zhu/aaron-marketing-skills/blob/5a1b2360eb9b61e8fb94e9403ff5f399ff40f765/seo-geo/evaluate/domain-authority-auditor/SKILL.md)
