...
The eight auditor-class skills use [`auditor-runbook.md`](auditor-runbook.md), [`audit-artifact.schema.json`](https://github.com/aaron-he-zhu/aaron-marketing-skills/blob/5a1b2360eb9b61e8fb94e9403ff5f399ff40f765/references/audit-artifact.schema.json), and the typed scorer. Their handoff adds framework/profile/catalog identity, target/date, typed context, coverage/confidence, score state, verdict/veto/cap, and allowed raw/final scores; durable run identity requires exact `catalog_version` plus non-empty strict-JSON `context`.
